...
ALL_PERMS = ["toggle_bot", "stats", "users", "add_config", "add_special_config", "broadcast", "coins", "channels", "support_id", "texts", "buttons"]
PERM_NAMES = {
    "toggle_bot":         "🔴🟢 خاموش/روشن ربات",
    "stats":              "📊 آمار کلی",
    "users":              "👥 لیست کاربران",
    "add_config":         "➕ افزودن کانفیگ",
    "add_special_config": "💎 افزودن کانفیگ ویژه",
...
BUTTON_NAMES = {
    ...
    "admin_add_special_config": "دکمه افزودن کانفیگ ویژه",
    "admin_special_panel": "دکمه پنل اتصال ویژه",
    "main_special_config": "دکمه اتصال ویژه",
    "admin_broadcast": "دکمه ارسال همگانی",
    "admin_addcoins": "دکمه انتقال سکه",
    "admin_subcoins": "دکمه کسر سکه",
    "admin_resetcoins": "دکمه حذف همگانی سکه",
    ...
    "admin_referral_coins": "دکمه تنظیم سکه رفرال",
...
COINS_PER_REFERRAL       = _config.get("coins_per_referral", 1)
COINS_TO_GET_CONFIG      = _config.get("coins_to_get_config", 3)
REFERRALS_TO_GET_SPECIAL = _config.get("referrals_to_get_special", 26)
...
class AdminStates(StatesGroup):
    ...
    set_referral_coins             = State()
    waiting_special_config_count   = State()
    waiting_special_config_item    = State()
    set_special_referrals          = State()
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS special_configs (
        id INTEGER PRIMARY KEY AUTOINCREMENT, content TEXT NOT NULL,
        is_used INTEGER DEFAULT 0, used_by INTEGER DEFAULT NULL, used_at TEXT DEFAULT NULL)""")
    try:
        c.execute("ALTER TABLE users ADD COLUMN special_configs_received INTEGER DEFAULT 0")
    except:
        pass
    c.execute("""CREATE TABLE IF NOT EXISTS stats (key TEXT PRIMARY KEY, value INTEGER DEFAULT 0)""")
    for key in ("total_users", "configs_given", "total_referrals", "special_configs_given"):
...
# ==================== توابع کانفیگ ویژه ====================
def get_free_special_config():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT id, content FROM special_configs WHERE is_used = 0 LIMIT 1")
    row = c.fetchone(); conn.close(); return row

def mark_special_config_used(config_id, user_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE special_configs SET is_used=1, used_by=?, used_at=? WHERE id=?",
              (user_id, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), config_id))
    c.execute("UPDATE users SET special_configs_received=COALESCE(special_configs_received,0)+1 WHERE user_id=?", (user_id,))
    conn.commit(); conn.close()

def add_special_config(content):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT INTO special_configs (content) VALUES (?)", (content.strip(),))
    conn.commit(); conn.close()

def get_special_configs_count():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT COUNT(*) FROM special_configs WHERE is_used = 0")
    free = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM special_configs")
    total = c.fetchone()[0]; conn.close(); return free, total

def get_sp_configs_received(user_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT COALESCE(special_configs_received, 0) FROM users WHERE user_id = ?", (user_id,))
    row = c.fetchone(); conn.close()
    return row[0] if row else 0

def get_special_config_users(page=0, per_page=20):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""SELECT DISTINCT u.user_id, u.full_name, u.username, u.is_banned,
                 COALESCE(u.special_configs_received, 0)
                 FROM users u
                 WHERE COALESCE(u.special_configs_received, 0) > 0
                 LIMIT ? OFFSET ?""", (per_page, page * per_page))
    rows = c.fetchall()
    c.execute("SELECT COUNT(*) FROM users WHERE COALESCE(special_configs_received, 0) > 0")
    total = c.fetchone()[0]; conn.close(); return rows, total
...
def main_keyboard(show_admin_btn: bool = False) -> InlineKeyboardMarkup:
    rows = [
        ...
        [InlineKeyboardButton(text=get_button_text("main_special_config"), callback_data="get_special_config", style=ButtonStyle.PRIMARY)],
        [
...
def admin_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    if row2:
        rows.append(row2)
    if has_perm(user_id, "add_special_config"):
        rows.append([
            InlineKeyboardButton(text=get_button_text("admin_add_special_config"), callback_data="admin_add_special_config"),
            InlineKeyboardButton(text=get_button_text("admin_special_panel"), callback_data="admin_special_panel_0"),
        ])
    row3 = []
    ...
    if row3:
        rows.append(row3)
    if has_perm(user_id, "coins"):
        rows.append([InlineKeyboardButton(text=get_button_text("admin_resetcoins"), callback_data="admin_resetcoins_ask", style=ButtonStyle.DANGER)])
        rows.append([InlineKeyboardButton(text=get_button_text("admin_referral_coins"), callback_data="admin_set_referral_coins")])
...
# ==================== بررسی عضویت در callback ها ====================
async def require_membership(call: CallbackQuery) -> bool:
    if is_any_admin(call.from_user.id):
        return True
    is_member = await check_membership(call.from_user.id, bot)
    if not is_member:
        try:
            await call.message.edit_text(get_bot_text("join_required_short"), reply_markup=join_keyboard())
        except:
            await call.answer("❌ برای استفاده باید عضو کانال باشی!", show_alert=True)
        return False
    return True
    ...
    if referred_by_credited:
        try:
            ...
            await bot.send_message(
                referred_by_credited,
                f"🎉 یه نفر با لینک دعوت تو وارد ربات شد و عضو کانال شد!\n"
                f"👤 نام: {user.full_name}\n"
                f"🔗 لینک دعوت اون: {ref_link}\n"
                f"+{COINS_PER_REFERRAL} سکه به حسابت اضافه شد 🪙"
            )
        except: pass
...
async def cb_get_config(call: CallbackQuery):
    ...
    if not await require_membership(call): return
...
# ==================== get_special_config ====================
@dp.callback_query(F.data == "get_special_config")
async def cb_get_special_config(call: CallbackQuery):
    await call.answer()
    user = call.from_user
    if not BOT_ENABLED and not is_any_admin(user.id):
        await call.message.edit_text("🔴 ربات درحال بروزرسانی هست\nبزودی روشن می‌شود 🙏"); return
    ...
    if ref_count < REFERRALS_TO_GET_SPECIAL:
        needed = REFERRALS_TO_GET_SPECIAL - ref_count
        await call.message.edit_text(
            f"💎 اتصال ویژه\n━━━━━━━━━━━━━━━\n\n"
            f"📦 حجم آنی کانفیگ: 500 مگ\n\n"
            f"❌ رفرال کافی نداری!\n\n"
            f"👥 رفرال‌های فعلی تو: {ref_count}\n"
            f"🎯 نیاز داری: {REFERRALS_TO_GET_SPECIAL} رفرال\n"
            f"📉 کمبود داری: {needed} رفرال\n\n"
            f"دوستاتو دعوت کن تا اتصال ویژه بگیری! 👇",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=get_button_text("get_referral"), callback_data="referral", style=ButtonStyle.PRIMARY)],
                [InlineKeyboardButton(text=get_button_text("back_main"), callback_data="back_main")],
            ])); return

    special_config = get_free_special_config()
    if not special_config:
        await call.message.edit_text(
            "💎 اتصال ویژه\n━━━━━━━━━━━━━━━\n\n"
            "😔 متأسفانه در حال حاضر کانفیگ ویژه موجود نیست.\nکمی صبر کن!",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=get_button_text("back_main"), callback_data="back_main")]])); return

    mark_special_config_used(special_config[0], user.id)
    increment_stat("special_configs_given")
    await call.message.edit_text(
        f"💎 اتصال ویژه تو اینه رفیق!\n\n"
        f"📦 حجم آنی: 500 مگ\n\n"
        f"<code>{special_config[1]}</code>",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=get_button_text("back_main"), callback_data="back_main")]]))
...
async def cb_admin_stats(call: CallbackQuery):
    ...
    free_special, total_special = get_special_configs_count()
    special_given = get_stat("special_configs_given")
    ...
    await call.message.edit_text(
        ...
        f"✅ کانفیگ های موجود فعلی: {free_configs}\n"
        f"🟢 کانفیگ‌های مصرف‌شده امروز: {configs_today}\n"
        f"🔴 کل کانفیگ‌های مصرف‌شده: {total_used}\n\n"
        f"💎 کانفیگ‌های ویژه موجود: {free_special} از {total_special}\n"
        f"💎 کانفیگ‌های ویژه داده‌شده: {special_given}\n"
        f"🔗 رفرال برای ویژه: {REFERRALS_TO_GET_SPECIAL}\n\n"
        f"🪙 سکه رفرال: {COINS_PER_REFERRAL} | سکه برای کانفیگ: {COINS_TO_GET_CONFIG}",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=get_button_text("back_panel"), callback_data="open_admin_panel")]]))

# ==================== حذف همگانی سکه ====================
@dp.callback_query(F.data == "admin_resetcoins_ask")
async def cb_resetcoins_ask(call: CallbackQuery):
    if not has_perm(call.from_user.id, "coins"):
        await call.answer("❌ دسترسی ندارید.", show_alert=True); return
    await call.answer()
    await call.message.edit_text(
        "⚠️ حذف همگانی سکه\n━━━━━━━━━━━━━━━\n\n"
        "این عملیات سکه تمام کاربران را صفر می‌کند!\n\n"
        "آیا مطمئن هستید؟",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ بله، سکه‌ها را صفر کن", callback_data="admin_resetcoins_confirm", style=ButtonStyle.DANGER)],
            [InlineKeyboardButton(text="❌ خیر، لغو", callback_data="open_admin_panel")],
        ]))

@dp.callback_query(F.data == "admin_resetcoins_confirm")
async def cb_resetcoins_confirm(call: CallbackQuery):
    if not has_perm(call.from_user.id, "coins"):
        await call.answer("❌ دسترسی ندارید.", show_alert=True); return
    await call.answer()
    reset_all_coins()
    await call.message.edit_text(
        "✅ سکه همه کاربران صفر شد!",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=get_button_text("back_panel"), callback_data="open_admin_panel")]]))

# ==================== تنظیم سکه رفرال ====================
@dp.callback_query(F.data == "admin_set_referral_coins")
async def cb_set_referral_coins(call: CallbackQuery, state: FSMContext):
    if not has_perm(call.from_user.id, "coins"):
        await call.answer("❌ دسترسی ندارید.", show_alert=True); return
    await call.answer()
    await state.set_state(AdminStates.set_referral_coins)
    await call.message.edit_text(
        f"🪙 تنظیم سکه رفرال\n━━━━━━━━━━━━━━━\n\n"
        f"سکه فعلی برای هر دعوت: {COINS_PER_REFERRAL}\n"
        f"سکه فعلی برای دریافت کانفیگ: {COINS_TO_GET_CONFIG}\n"
        f"رفرال فعلی برای اتصال ویژه: {REFERRALS_TO_GET_SPECIAL}\n\n"
        f"عدد جدید سکه رفرال رو بفرست (مثلاً 2):\n"
        f"بعد از اون سکه مورد نیاز برای کانفیگ و رفرال ویژه رو هم می‌پرسم.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=get_button_text("cancel"), callback_data="admin_cancel")]]))

@dp.message(AdminStates.set_referral_coins)
async def hdl_set_referral_coins(message: Message, state: FSMContext):
    global COINS_PER_REFERRAL, COINS_TO_GET_CONFIG, REFERRALS_TO_GET_SPECIAL
    if not has_perm(message.from_user.id, "coins"):
        await state.clear(); return
    data = await state.get_data()
    step = data.get("ref_coin_step", 1)

    if not message.text.strip().isdigit() or int(message.text.strip()) <= 0:
        await message.answer("❌ عدد معتبر (بزرگتر از صفر) بفرست:"); return

    if step == 1:
        COINS_PER_REFERRAL = int(message.text.strip())
        await state.update_data(ref_coin_step=2, new_ref=COINS_PER_REFERRAL)
        await message.answer(
            f"✅ سکه رفرال روی {COINS_PER_REFERRAL} تنظیم شد.\n\n"
            f"حالا عدد سکه مورد نیاز برای دریافت کانفیگ رو بفرست (مثلاً 3):")
    elif step == 2:
        COINS_TO_GET_CONFIG = int(message.text.strip())
        await state.update_data(ref_coin_step=3)
        await message.answer(
            f"✅ سکه کانفیگ روی {COINS_TO_GET_CONFIG} تنظیم شد.\n\n"
            f"حالا تعداد رفرال مورد نیاز برای اتصال ویژه رو بفرست (فعلاً {REFERRALS_TO_GET_SPECIAL}):")
    else:
        REFERRALS_TO_GET_SPECIAL = int(message.text.strip())
        cfg = load_config()
        cfg["coins_per_referral"] = COINS_PER_REFERRAL
        cfg["coins_to_get_config"] = COINS_TO_GET_CONFIG
        cfg["referrals_to_get_special"] = REFERRALS_TO_GET_SPECIAL
        save_config(cfg)
        await state.clear()
        await message.answer(
            f"✅ تنظیمات ذخیره شد!\n\n"
            f"🪙 سکه به ازای هر دعوت: {COINS_PER_REFERRAL}\n"
            f"🎯 سکه مورد نیاز برای کانفیگ: {COINS_TO_GET_CONFIG}\n"
            f"💎 رفرال برای اتصال ویژه: {REFERRALS_TO_GET_SPECIAL}",
...
# ==================== افزودن کانفیگ ویژه ====================
@dp.callback_query(F.data == "admin_add_special_config")
async def cb_add_special_config(call: CallbackQuery, state: FSMContext):
    if not has_perm(call.from_user.id, "add_special_config"): return
    await call.answer()
    free, total = get_special_configs_count()
    await state.set_state(AdminStates.waiting_special_config_count)
    await call.message.edit_text(
        f"💎 افزودن کانفیگ ویژه\n\n"
        f"کانفیگ‌های ویژه فعلی: {free} موجود از {total} کل\n\n"
        f"چند تا کانفیگ ویژه می‌خوای اضافه کنی؟\nعدد بفرست:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=get_button_text("cancel"), callback_data="admin_cancel")]]))

@dp.message(AdminStates.waiting_special_config_count)
async def hdl_special_config_count(message: Message, state: FSMContext):
    if not has_perm(message.from_user.id, "add_special_config"): return
    if not message.text.strip().isdigit() or int(message.text.strip()) <= 0:
        await message.answer("❌ عدد معتبر بفرست:"); return
    count = int(message.text.strip())
    await state.update_data(special_config_count=count, special_config_received=0)
    await state.set_state(AdminStates.waiting_special_config_item)
    await message.answer(f"✅ {count} تا کانفیگ ویژه ثبت می‌کنیم.\n\nکانفیگ ویژه شماره 1 رو بفرست:")

@dp.message(AdminStates.waiting_special_config_item)
async def hdl_special_config_item(message: Message, state: FSMContext):
    if not has_perm(message.from_user.id, "add_special_config"): return
    data = await state.get_data()
    add_special_config(message.text.strip())
    received = data["special_config_received"] + 1
    await state.update_data(special_config_received=received)
    if received >= data["special_config_count"]:
        await state.clear()
        await message.answer(
            f"✅ همه {data['special_config_count']} کانفیگ ویژه اضافه شدن!",
            reply_markup=admin_keyboard(message.from_user.id)); return
    await message.answer(f"✅ کانفیگ ویژه {received} ثبت شد.\n\nکانفیگ ویژه شماره {received+1} رو بفرست:")

# ==================== پنل اتصال ویژه ====================
@dp.callback_query(F.data.startswith("admin_special_panel_"))
async def cb_admin_special_panel(call: CallbackQuery):
    if not has_perm(call.from_user.id, "add_special_config"):
        await call.answer("❌ دسترسی ندارید.", show_alert=True); return
    await call.answer()
    try:
        page = int(call.data.split("_")[-1])
        ...
        page = 0
    free_sp, total_sp = get_special_configs_count()
    special_given = get_stat("special_configs_given")
    users, total_users = get_special_config_users(page, 20)

    buttons = []
    for u in users:
        banned = "🚫 " if u[3] else ""
        name = u[1] or f"کاربر {u[0]}"
        buttons.append([InlineKeyboardButton(
            text=f"{banned}{name} | 💎{u[4]}",
            callback_data=f"admin_sp_userdetail_{u[0]}"
        )])
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text=get_button_text("prev_page"), callback_data=f"admin_special_panel_{page-1}"))
    if (page + 1) * 20 < total_users:
        nav.append(InlineKeyboardButton(text=get_button_text("next_page"), callback_data=f"admin_special_panel_{page+1}"))
    if nav:
        buttons.append(nav)
    buttons.append([InlineKeyboardButton(text=get_button_text("back_panel"), callback_data="open_admin_panel")])
    ...
    await call.message.edit_text(
        f"💎 پنل اتصال ویژه\n━━━━━━━━━━━━━━━\n\n"
        f"📦 کانفیگ‌های ویژه موجود: {free_sp} از {total_sp} کل\n"
        f"🎁 کانفیگ‌های ویژه داده‌شده: {special_given}\n"
        f"🔗 رفرال مورد نیاز: {REFERRALS_TO_GET_SPECIAL}\n\n"
        f"👥 کاربران دریافت‌کننده ({total_users} نفر):",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))

@dp.callback_query(F.data.startswith("admin_sp_userdetail_"))
async def cb_sp_user_detail(call: CallbackQuery):
    if not has_perm(call.from_user.id, "add_special_config"): return
    await call.answer()
    target_id = int(call.data.split("_")[-1])
    u = get_user_detail(target_id)
    ...
    sp_received = get_sp_configs_received(target_id)
    status = "🚫 مسدود" if u[4] else "✅ فعال"
    ban_btn = (InlineKeyboardButton(text=get_button_text("unban_user"), callback_data=f"admin_unban_{target_id}")
               if u[4] else InlineKeyboardButton(text=get_button_text("ban_user"), callback_data=f"admin_ban_{target_id}"))
    await call.message.edit_text(
        f"💎 کاربر اتصال ویژه\n━━━━━━━━━━━━━━━\n"
        ...
        f"👥 رفرال: {ref_count}\n"
        f"📌 وضعیت: {status}\n"
        f"🎁 کانفیگ ویژه دریافتی: {sp_received}\n━━━━━━━━━━━━━━━",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [ban_btn],
            [InlineKeyboardButton(text=get_button_text("msg_user"), callback_data=f"admin_msguser_{target_id}")],
            [InlineKeyboardButton(text=get_button_text("send_coin_user"), callback_data=f"admin_addcoin_{target_id}"),
             InlineKeyboardButton(text=get_button_text("sub_coin_user"), callback_data=f"admin_subcoin_{target_id}")],
            [InlineKeyboardButton(text="🔙 بازگشت به پنل ویژه", callback_data="admin_special_panel_0")],
        ]))
